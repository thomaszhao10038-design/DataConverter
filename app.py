...
def process_sheet(df, timestamp_col, psum_col):
    ...
    for date in grouped["Date"].unique():
        ...
        if non_zero_mask.any():
            ...
            # Set kW and PSumW to pd.NA (empty cell) for leading and trailing padded zeros
...
def build_output_excel(sheets_dict):
    ...
    for sheet_name, df in sheets_dict.items():
        ...
        for date in dates:
            ...
            # Note: We reference the full possible range for consistency, but the NA values will be ignored by the chart
        ...
        if chart_series_list:
            ...
            # The categories should be explicitly plotted on the X-axis
            chart.series = chart_series_list # Add series before setting X-axis
            chart.set_categories(categories_ref)
...
def app():
    ...
    st.markdown("Upload your Excel file containing high-resolution power data. The app will resample it into 10-minute intervals and apply **zero-trimming**: leading/trailing zero-padded values (missing readings) are removed, while internal zero readings are kept. The output includes daily tables, charts, and summaries.")
