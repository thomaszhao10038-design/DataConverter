...
def build_output_excel(sheets_dict):
    ...
    # Write Data Rows
    for r_idx, date_val_str in enumerate(all_dates, start=2):
        # Convert YYYY-MM-DD string to datetime object and apply Excel date format for cleaner chart labels
        date_obj = datetime.strptime(date_val_str, '%Y-%m-%d').date() 
        date_cell = ws_total.cell(row=r_idx, column=1, value=date_obj) 
        date_cell.number_format = 'dd/mm/yy'
        
        total_load = 0
        for c_idx, sheet_name in enumerate(sheet_names, start=2):
            value = total_data[date_val_str].get(sheet_name, 0) 
    ...
    # The existing logic is retained as it plots based on the table, and uses 
    # the sheet names (from row 1) as the legends (series titles) correctly.
    # -----------------------------
    ...
    # min_row=1 ensures column header (Sheet Name/Total Load) is used as the legend title
    for c_idx, sheet_name in enumerate(sheet_names, start=2):
