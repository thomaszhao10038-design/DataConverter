...
def build_output_excel(sheets_dict):
    ...
    # -----------------------------
    # Line chart for Total sheet (daily plot connected by line)
    ...
    data_ref_total = Reference(ws_total, min_col=2+len(sheet_names), min_row=1, max_row=1+len(all_dates))
    series_total = Series(data_ref_total, title_from_data=True)
    chart_total.series.append(series_total)

    categories_ref = Reference(ws_total, min_col=1, min_row=2, max_row=1+len(all_dates))
    chart_total.set_categories(categories_ref)
...
def app():
    ...
    st.markdown("""
        ...
        The output Excel file includes a **line chart**, a **Max Power Summary table**, and a **Total sheet** with daily max power for all sheets and a line chart of all sheets + total load connected by daily lines.
    """)
